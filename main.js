...
function initValidation(holder) {
	...
	function setState(hold, field, error) {
		...
		if (error) {
			...
			field.one('click',function(){hold.removeClass(errorClass).removeClass(successClass);});
			successFlag = false;
		} else {
		...
		function update() {
			for(var i = 0; i < mp; i++) {
				...
				p.y -= Math.cos(p.d) + 1 + p.r/2;
					...
					{
						particles[i] = {x: Math.random() * winWidth, y: winHeight + 50, r: p.r, d: p.d};
					}
				}
			}
		}
...
;(function($) {
	...
	CustomPopup.prototype = {
		...
		loadPopup: function() {
			...
			$.get(this.opener.attr('popup-src'), function(source) { self.successHandler(source); });
		},
	...
	$.fn.customPopup = function(opt) {
		return this.each(function() {
			jQuery(this).data('CustomPopup', new CustomPopup($.extend(opt, { holder: this })));
		});
	};
}(jQuery));
